...
После долгого бездействия GitHub Pages страница ставит локальный мост на паузу. Это не полная блокировка: если токен текущей вкладки ещё есть в `sessionStorage`, страница покажет кнопку восстановления и спросит подтверждение перед новым подключением к `127.0.0.1`.
Автоматического переподключения без клика пользователя нет.
